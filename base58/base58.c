...
bool b58enc(char *b58, size_t *b58sz, const void *bin, size_t binsz)
{
    ...
    const uint8_t *binu = bin;
    uint8_t *buf = (uint8_t*)alloca(binsz * 2);

    for (i = 0; i < binsz && !binu[i]; ++i)
        ++zcount;

    size = (binsz - i) * 138 / 100 + 1;
    memset(buf, 0, size);

    for (high = size - 1; i < binsz; ++i, high = j)
    {
        for (carry = binu[i], j = size - 1; (j > high) || carry; --j)
        {
            carry += 256 * buf[j];
            buf[j] = carry % 58;
            carry /= 58;
...
void *search_private_key(void *threadarg) {
    thread_data_t *my_data;
    my_data = (thread_data_t *) threadarg;
    int taskid = my_data->thread_id;

    // Implemente a lógica de busca da chave privada aqui
...
int main() {
    ...
    for (t = 0; t < NUM_THREADS; t++) {
        ...
        rc = pthread_create(&threads[t], NULL, search_private_key, (void *)&thread_data[t]);
        if (rc) {
